...
```typescript
// Lock collateral (user signs via Freighter)
...
<Tip>
Any Stellar address can be registered as an anchor by the protocol admin. The `register_anchor` function supports upsert logic — re-registering an existing anchor updates its credit limit without losing existing state.
</Tip>
